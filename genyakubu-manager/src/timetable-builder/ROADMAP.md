...
最終更新: **2026-07-16** — **§R: 配布用 Excel を完成版レイアウト
(例年の掲示紙面の再現、distributionExport.ts) に置換**。
それ以前 (2026-07-04): **§O: 時間割本体の日付行をカレンダー順で表示する
修正 (ユーザ報告)** を追加。それ以前 (2026-07-03):
**§L (新機能・便利機能レビュー、4 方面並列調査)
...
3. ~~コードの軽い一手なら E3c か E4b~~ ✅ 両方完了 (2026-07-03。E1h 主要分・
   ...
   **新規の修正・改善候補は §K (2026-07-03 棚卸し) と §L (同日、新機能・
   ...
   ブラッシュアップレビュー) — **全 35 件実装済み (2026-07-04 完了)**
...
- ✅ **L5c (2026-07-03)**: 配布用 (注記なし) Excel — `buildScheduleWorkbook`
  ...
  テスト +3
  **→ 2026-07-16 (§R1) に完成版レイアウト (distributionExport.ts) へ置換して
  clean オプションは廃止** (注記なし学年グリッドという形の配布用は無くなった)
...
**実装状況 (2026-07-03〜04、8 バッチで全 35 件を実装済み・全件 ✅ 付与済み)**。
...
- ✅ **N2a (2026-07-04 MVP 実装)**: セルの複数選択 (Shift+クリック / ドラッグ矩形) →
  選択範囲の一括クリア・ロック・貼り付け。現状は 1 セル単位 or ヘッダ単位
  (日/時限/クラスまるごと) しかなく、「クラス A〜C の 3〜4 限だけロック」の
  ような矩形操作ができない。まず範囲クリア・範囲ロックの MVP から。
  **実装メモ**: Ctrl(⌘)+クリックでトグル選択、Shift+クリックでアンカーから
  矩形選択 (utils/cellSelection.computeRectKeys)。選択中は一括操作バー
  (🧹 クリア / 🔒 ロック / 🔓 解除、いずれも atomic = Undo 1 回、クリアは
  ロック温存 + 合同 cleanup で単一 clear と同意味論)。Esc / タブ切替で解除。
  範囲貼り付け・キーボードでの範囲拡張 (Shift+矢印) は後続候補
...
- ✅ **N2e (2026-07-03 実装)**: タブ削除 confirm に割当数を出す — 現状は汎用文言のみ
  ...
  同型) に
- ✅ **N2f (2026-07-04 実装)**: undo/redo の内容可視化 — 変更セルが画面外だと押しても
...
- ✅ **N3a (2026-07-03 実装)**: 講師別の連続コマ上限 — 連続だけ project 全体値
  ...
  既存パターンの踏襲
- ✅ **N3b (2026-07-04 実装)**: 部分解の未充填セルに「なぜ埋まらないか」を付ける —
...
- ✅ **N3d (2026-07-03 実装)**: 「この案を採用」前の差分表示 — 採用は無確認即実行
...
- ✅ **N3f (2026-07-03 可視化部分を実装)**: 飛び石出勤の可視化 — 「その日 1 コマだけ出勤」を避ける
  仕組みも気付く指標も無い (制約は上限方向のみ)。まず案カードに「1 コマ
  出勤日を持つ講師 N 名」の情報表示 (solver 変更不要)、発展は L3c ソフト
  希望の枠組みで「出勤日集約」を重みに。**可視化 (案カードの 🚶 表示 + summarizeSingleSlotDays) は実装済み、ソフト制約化は L3c と同時に検討**
- ✅ **N3g (2026-07-03 実装)**: 講師コマ数を「使用/上限」表記に — SummaryPanel の
...
- ✅ **N4a (2026-07-03 実装)**: テンプレートの上書き更新と同名ガード — 保存は常に新規追加
  (templates.ts addTemplate)。年度ベースを微修正するたび同名が重複蓄積し、
  一覧から見分けられない。「上書き / 新規」の選択 + 行内「現在の内容で更新」
- ✅ **N4b (2026-07-03 実装)**: CSV 講師取込に「新規 X / 更新 Y」内訳を表示 — 親取込 (L5a)
  は confirm に内訳を出すのに CSV は合算 toast のみ (TeacherManager.tsx)。
  N1c の空 subjects 警告と同じ突き合わせで実装できる
- ✅ **N4c (2026-07-03 実装)**: 外部セッションプリセットに講師リストを保持 — 現状は
  時刻/期間/メモのみで、適用のたび対象講師を選び直す (AbsenceNgPanel.tsx)。
  「予備校 = 毎回同じ講師群」が典型なので任意で講師も保存 (存在しない講師は
  無視 + warning)
- ✅ **N4d (2026-07-03 実装)**: 講師別上限の矛盾表示 — 1日 > 通算 (例: 1日8・通算3) を
  無警告で受理 (TeacherManager.tsx)。行内の注意アイコン/tooltip で十分
- ✅ **N4e (2026-07-03 実装)**: コマ数マトリクスに列合計 — 学年ごとの総コマ数がどこにも
  出ず暗算が必要 (SubjectManager.tsx)。「計 N / 収容 M (使う日×時限×クラス)」
  を列に常時表示すれば入れ過ぎ/不足に設定段階で気づける
- ✅ **N4f (2026-07-03 TeacherManager 分を実装)**: ConfigModal 横断の講師名フィルタ — TeacherManager /
  ClassPriority / NG マトリクス / クイックグリッドが講師 30 名規模で縦横に
  長く、絞り込み手段が無い。名前 input 1 つで横断的に効く (明示操作なので
  A18 非該当)。**TeacherManager の一覧は実装済み。ClassPriority / NG
  マトリクス / クイックグリッドへの横展開は残**
- ✅ **N4g (2026-07-22 実装)**: プリセット改名の登録済みセッション同期 — プリセットは
  適用時のコピーでセッションにプリセット ID が残らないため、名称の間違いを
  後から直しても登録済みメモは古いまま (「予備校1限が実は2限」ケース)。
  `preset/update` に `syncSessionMemos` フラグを追加し、編集フォームに
  「登録済み N 件のメモも更新」チェック (既定 ON、対象がある時のみ表示) を
  出す。突き合わせは `utils/presetRenameSync` (変更前ラベル + 時刻の完全
  一致のみ。時刻も見るので連鎖リネームで直前リネーム分を巻き込まない)。
  preset とセッションを同一 action で更新するので Undo 1 回で両方戻る

### N5. 出力・オンボーディング・可視化

- ✅ **N5a (2026-07-03 実装)**: Excel 学年グリッドシートにタイトル行が無い — 1 行目が
...
- ✅ **N5b (2026-07-03 実装)**: オンボーディングが出力に触れない — 5 ステップは編集・設定・
  ...
  保存) への 1 ステップを追加
- ✅ **N5c (2026-07-03 実装)**: 保存容量の常時表示 — 警告は起動時 1 回・ratio≥0.5 の toast
  のみで、それ以外は情報ゼロ (BuilderApp.tsx)。ヘッダ or ⚙️ に「保存データ
  約 X MB」(formatBytes 流用)。N1g とセットで
- ✅ **N5d (2026-07-03 実装)**: 講師別 Excel の列見出し統一 — 個人シート「場所(タブ)」vs
  全講師リスト「タブ名」の揺れ (excelExport.ts)。「学年(タブ)」等に統一

---

## O. 2026-07-04 ユーザ報告の修正

- ✅ **O1 (2026-07-04 修正)**: 時間割本体の日付行がカレンダー順にならない —
  日付プール (project.dates) は挿入順で、タブ別に後から追加した日は
  `tabDates/setByLabels` が末尾 push するため、全タブ共通の日が先・後付けの
  日が後ろに並ぶ (7/29〜8/4 の後に 7/24 が来る等。F5j と同根で、F5j は
  NG パネル表示のみの対処だった)。回数連番 (①②…、analysisHelpers の
  subjectOrders) も同じ配列順で数えるため「時系列で前の日なのに回数が後」の
  ずれも起きていた。
  **修正**: 実効 config の唯一の入口 `activeDatesForTab` (scheduleKey.ts) で
  subset を `sortPoolDatesByCalendar` に通してカレンダー順で返すようにした。
  時間割表示・回数連番・Excel 出力・自動生成・分析の全 consumer
  (effectiveConfigForTab 経由) が一括で日付順になる。プールの保存順=挿入順は
  不変 (JSON / fingerprint に影響なし)。M/D として解釈できないラベルは
  末尾 (安定順)、既に日付順なら参照をそのまま返す (identity 前提の
  memo / no-op 判定を保つ)。periods は手打ちの並びを尊重してプール順のまま。
  回帰テスト +4 (scheduleKey.test.js)

---

## P. 2026-07-09 ユーザ要望 (講師別 Excel の印刷デフォルト)

- ✅ **P1 (2026-07-09 実装)**: 講師別 Excel を「開いてそのまま印刷」できる
  デフォルトにしたい (B4 縦・両面長辺綴じ・一番上に講師名) — 従来は
  ヘッダ行始まりで、用紙・向きもプリンタ既定のままだった (excelExport.ts)。
  **実装**: 各講師シートの先頭に「講師名 — プロジェクト名 / 期間 (その講師の
  初日〜最終日) / 出力日」のタイトル行 (学年グリッドの N5a と対)。全シート
  (個人 + 全講師リスト) の pageSetup に B4 縦を埋め込み、タイトル/ヘッダ行は
  Print_Titles で 2 ページ目以降にも繰り返し印刷。オートフィルタ・日付区切り
  太線は 1 行下がって維持。テスト +6、既存の行オフセット参照を更新。
  **両面 (長辺綴じ) はファイルに保存できない**: OOXML の pageSetup に duplex
  属性が無く、プリンタドライバ固有の DEVMODE バイナリ
  (xl/printerSettings/*.bin) にしか載らない。exceljs 非対応かつ埋め込んでも
  開く端末・プリンタが変わると効かないため、両面はプリンタ側の既定設定で
  運用する (DEVMODE 埋め込み案は再検討しない)。
- ✅ **P2 (2026-07-10 実装)**: 全講師リストシートも「開いてそのまま印刷」
  できるデフォルトにしたい — ユーザが手で整えた出力ファイル (講師の
  切り替わりに手動改ページ + 印刷範囲 A:G) と同じ設定を出力時に埋め込む
  (excelExport.ts)。
  **実装**: 講師が切り替わる行の直前 (前講師の最終行) に手動改ページを
  入れ、通しで印刷しても講師ごとに新しいページから始まるようにした
  (同一講師内の日付切り替わりでは改めない。ヘッダ行は P1 の Print_Titles で
  毎ページ載る)。印刷範囲は表の列 (全講師リスト!$A:$G) に固定。B4 縦は
  P1 で設定済み。テスト +4 (rowBreaks は exceljs の読み戻しで復元されない
  ため、書き出した zip 内の XML を JSZip で直接検証)。

---

## Q. 2026-07-14 ユーザ要望 (Excel に回数連番)

- ✅ **Q1 (2026-07-14 実装)**: 画面のセルに出る回数連番 (①②…、その科目が
  同一クラス内で何回目の授業か) が Excel 出力に載らない — 紙で配ると
  第N回が分からない (excelExport.ts)。
  **実装**: 全体 Excel (学年グリッド、配布用 clean 含む) と講師別 Excel
  (個人シート + 全講師リスト) の科目名に UI と同じ丸数字を付ける。
  数え方は computeActiveAnalysis の subjectOrders をそのまま使用
  (クラス内を日付 → 時限順に走査した 1-based 連番 = 画面表示と常に一致。
  makeSubjectOrderMarker に切り出し)。UI と同様、同一クラス×同日の重複
  (subjectDup 違反) 中は番号を出さない。クォータ超過の回は作業用の全体
  Excel のみ '!' を添える (UI の赤字に対応する作成者向け注記。配布用・
  講師別では省く)。科目別集計シートは対象外。テスト +7、合同の既存
  期待値 +①。

---

## R. 2026-07-16 ユーザ要望 (配布用 Excel の完成版レイアウト)

- ✅ **R1 (2026-07-16 実装)**: 最終の完成版配布用 Excel を、例年手作業で
  仕上げていた掲示紙面 (2025 年夏期の完成版 xls をサンプルとして受領) と
  同じフォーマットにしたい — 従来の配布用 (L5c: 注記なし学年グリッド) は
  作業表の見た目のままで、掲示用には作り直しが必要だった。
  **実装**: 新モジュール `utils/distributionExport.ts` の
  `buildDistributionWorkbook` が元紙面を再現する:
  - 1 シート = 同じ時限セット (effective periods) を使うタブのグループ
    (例: 中1+中2 の夜帯で 1 枚、中3 の昼帯で 1 枚)。シート名は
    「タブ名…時間割」
  - 日付ブロックを縦積みし左列 → 右列の 2 段組 (新聞式)。各時限は
    「時限名+クラス別科目」「時刻+クラス別講師」の 2 行ペア。日付セルは
    ブロック全高に結合して「M/D\n(曜)」
  - 先頭にクラス行 + 教室行 (複数タブなら学年行も)。隣接する同名クラス
    (教室違い) は見出しを結合。**教室行はモデルに情報が無いので空欄**
    (塗り+罫線だけ用意して Excel 上で手書き)
  - タブ内全クラスが同一科目 + 講師なしの時限 (確認テスト) は 2 行 ×
    クラス列を 1 セルに結合 (判定は連番抜き、隣接タブが同文言なら跨いで
    結合)。授業ゼロの日は時限欄ごと 1 つの空セルに結合 (イベント説明の
    書き込み欄)
  - 書式は元紙面の実測値: ＭＳ Ｐゴシック / 講師は明朝、時限ペア間 hair・
    ブロック外周 thin の罫線、シート順に循環するアクセント色 (薄緑 → 水色
    → …)、A4 縦 1×1 の fit-to-page。科目の色分け・稼働カウント・⚠NG・
    超過 '!' は載せない ('未定' は空欄)
  ...
  **モデル外の情報 (教室番号・確認テストの出題範囲・イベント日の説明文・
  ...
  別途要件から (現状は年 1 回書く程度なので手書きで十分という判断)。

---

## S. 2026-07-28 ユーザ要望 (講師別 Excel にクラス別回数まとめ)

- ✅ **S1 (2026-07-28 実装)**: あるクラスの授業に結局どの講師が何回行くかを
  まとめて見たい — 座席表を毎回提出する運用のため、講師×クラスの回数が
  最初にまとまっていれば座席表を必要枚数あらかじめ印刷しておける
  (excelExport.ts)。
  **実装**: 講師別 Excel の 1 枚目に「クラス別回数」シートを追加。
  行 = 講師 (project.teachers 順、登録外の名前は後ろ、講師未割当は
  '(未定)' で末尾)、列 = タブ順 × クラス順の行列で、各セルはそのクラスへ
  行く回数 (0 は空欄)。右端に行の計 (= その講師が実際に行く回数)、最下段に
  列の計 (= クラスの総コマ数。(未定) を含むので割当の進み具合の
  突き合わせにも使える)。合同は科目別シートと同じ「代表クラスに 1 回」の
  規約、外部授業 (他学年セッション) は講習のクラスに行くものではないので
  対象外。ヘッダは学年(タブ)/クラスの 2 段 (タブ名は横結合、同名タブでも
  span は分ける)、B4 縦 + 先頭 3 行の繰り返し (P1 と同じ既定)。該当が
  ある場合のみ最下部に凡例 (合同・(未定) の説明) を注記。集計は純粋関数
  `computeTeacherClassCounts` に切り出してテスト。テスト +9 (round-trip
  検証含む)、F5i の枚数期待値を更新。
- ✅ **S2 (2026-07-28 実装)**: 確認テストの回数も S1 のまとめに載せたい —
  各日の最終コマは「授業時間 + 休憩 (中3 は 5 分、他は 10 分) + 確認テスト
  15 分」を含む長さで設計されており、最終コマの担当講師がそのまま確認
  テストを監督する。確認テストにも座席管理があるため、座席表と同様に
  回数が事前に分かると準備できる (excelExport.ts)。
  **実装**: クラス別回数シートの各セルに、うち確認テスト付き (= その日の
  最終コマ) の回数を「5(テ2)」の形で併記。計の列・合計行も同表記
  (テスト 0 のセルは素の数値のまま)。最終コマの判定は『タブが使う時限
  (実効時限リスト) の末尾の時限 ID』— 時限は日をまたいで共通なので
  ID 一致で判定できる (E-3 の絞り込みも通す)。テ表記がある場合のみ凡例に
  説明を追加。**確認テストの中身 (出題範囲等) はモデル外のまま** (R1 と
  同じ方針: 管理機能は必要になったら要件から)。テスト +3、S1 の期待値を
  テ表記に更新。
- ✅ **S3 (2026-07-28 実装、同日レイアウト変更)**: 個人シート (各講師の
  シート) にも自分の分のクラス別回数を載せたい — 印刷して個人に渡った紙
  1 枚で座席表・確認テストの必要枚数が分かるように (excelExport.ts)。
  **実装**: スケジュールリストの下 (空行 2 つ挟む) に、タブごとの小さな
  表ブロックを縦に積む。各ブロックは「タブ名 (クラス列ぶん横結合、講師別
  ヘッダの緑) → クラス名 (薄緑) → 回数 `5(テ2)`」の 3 行で、**行く
  クラス (回数 > 0) だけ**を出す。最後のブロックの右にはタブ名行〜クラス
  行を縦結合した「計」ヘッダ + 自分の総回数。集計は S1/S2 の
  `computeTeacherClassCounts` を 1 回だけ計算して共用し、ブロック構成は
  純粋関数 `buildTeacherCountBlocks` に切り出し。空行区切りにより
  オートフィルタの連続データ領域から外れるので、学年で絞り込んでも
  ブロックは巻き込まれない。
  **当初はタイトル直下 (行 2) の 1 行テキスト**
  (`クラス別回数: ３S 5(テ2)、…`) **で実装したが、担当クラスが多い講師では
  紙面で右に切れて読めないため、ユーザ提示のモックに合わせて同日中に
  リスト下のブロック形式へ変更** (1 行まとめ方式は再提案しない)。
  テスト +4、個人シートの行オフセット参照を全更新。
