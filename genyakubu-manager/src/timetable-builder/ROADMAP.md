...
3. ~~コードの軽い一手なら E3c か E4b~~ ✅ 両方完了 (2026-07-03。E1h 主要分・
   ...
   **新規の修正・改善候補は §K (2026-07-03 棚卸し) と §L (同日、新機能・
   ...
   ブラッシュアップレビュー) — うち 32 件は実装済み (バグ・実害系 N1a〜N1j
   含む)。残は N2a / N2f / N3b (§N 冒頭の実装状況を参照)
...
**実装状況 (2026-07-03〜04、7 バッチで 32/35 件を実装・全件 ✅ 付与済み)**。
残る 3 件はいずれも設計を伴う大きめの候補:

- **N2a** セルの複数選択 → 一括操作 (大。選択モデルの設計から)
- **N2f** undo/redo の内容可視化 (中。toast の配線先 = context 層の設計判断が要る)
- **N3b** 部分解の未充填セルへの理由付け (中〜大。候補判定の逆引き設計から)
