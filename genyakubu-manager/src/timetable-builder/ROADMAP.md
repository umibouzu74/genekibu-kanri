...
を新設**し、小粒バッチ **L1 (8 件) を実装** (テスト 1879 件)。L2〜L5 は
未着手候補として §L に整理。それ以前 (同日): §G.6 推奨順の小粒バッチ ×2 完了
...
3. ~~コードの軽い一手なら E3c か E4b~~ ✅ 両方完了 (2026-07-03。E1h 主要分・
   ...
   **新規の修正・改善候補は §K (2026-07-03 棚卸し) と §L (同日、新機能・
   便利機能レビュー) に集約** — K2a は修正済み。§L の L1 (小粒 8 件) も
   実装済みで、次の一手は L2〜L5 から価値順に選ぶ
   (推奨: L3a/L3b 講師別上限 → L5a 親マスタ取込 → L2a/L2c 編集系)
...
- **L2a (中・推奨)**: 講師ハイライト — 講師名クリック/選択でその講師の
  割当セルをグリッド上で強調。負荷調整時の集計パネルとの往復を解消
  (E1d 延期の「diff の直接ハイライト」とは別物)
- **L2b (中)**: 違反の巡回ナビ — popover の「→」が各種別の firstKey のみ。
  「次/前の違反へ」で全件を順に潰せるように (Toolbar.tsx:108-139)
- **L2c (中・推奨)**: 割当の反復・日列コピー — 「この割当を全日の同クラス
  同時限へ適用」+「この日の列を別の日へコピー」(K4a の label/index
  マッピングを流用)
...
### L3. ソルバ・制約の実務表現 (未着手候補)

- **L3a (中・推奨)**: 講師別の 1 日コマ数上限 — `Teacher.maxDailyHours?`
  (未設定は project 全体値へフォールバック)。「非常勤は 1 日 2 コマまで」
  が現状表現できない (teacherConstraints.ts:37-53 は単一値)
- **L3b (中・推奨)**: 講師別の通算コマ数上限 — 「講習トータル 8 コマまで」。
  solver は日次 tempDaily と同型の running total で弾く。週単位上限は
  dates に週概念が無いので後続
...
- **L3e (中・推奨)**: 部分解の未充填内訳 — 現状「159/168」+ 最初の詰まり
  1 セルのみで残りは目視探索。未充填一覧 + 科目/クラス/日別サマリ +
  「残りだけ再生成」導線 (bestPartial は手元にあるのに未活用)
- **L3f (中)**: 生成案の退避 — 未採用案をスナップショットへ保存して後で
  比較 (「この案をスナップショットに保存」)。案間 diff は E1d 延期の続き

### L4. 設定・入力の手間削減 (未着手候補)

- **L4a (中・推奨)**: コマ数マトリクスの一括入力 — 「この列 (タブ) を
  全科目◯コマ」「このタブの値を他タブへコピー」(SubjectManager は
  1 マスずつ入力のみ)
- **L4b (中)**: 日付プールの一括削除 — プールは増える一方で、掃除は
  1 日ずつ × 危険確認。「未使用の日をまとめて削除」
- **L4c (中)**: 講師 CSV の未登録科目に「マスタへ一括追加」ボタン
  (現状は赤字警告どまりで、見えない割当として取り込まれる)
- **L4d (小)**: 科目のカンマ区切り一括追加 (クラスは一括 textarea があるのに
  科目だけ 1 件ずつモーダル)
- **L4e (小)**: 日付自動生成のプレビュー (確定前に生成される日付一覧を表示)
- **L4f (小)**: CSV 雛形のダウンロード (現在の日付・時限・講師ラベル入り)
- **L4g (中)**: 「使う日/時限」選択の他タブコピー (K4a はスケジュールのみで
  日程選択は対象外)

### L5. 出力・親アプリ連携 (未着手候補)

- **L5a (中・推奨)**: 親アプリの講師マスタ取り込み — 親は partTimeStaff
  (name+subjectIds) を持つのに builder へは CSV 手入力のみで、二重入力 +
  表記ズレの温床。ユーザ発火の「親アプリから講師を取り込む」ボタン
  (明示操作なので A18 自動学習禁止に抵触しない)
- **L5b (中)**: 全学年一括印刷 — 現状はアクティブタブ 1 枚のみ
  (Excel「全体」は全タブ入りで非対称)。印刷時のみ全タブを隠しレンダリング
  or「この学年/全学年」トグル
- **L5c (中・推奨)**: 配布用 (注記なし) 出力 — Excel セルの
  「(中学3:計5)⚠NG」等の作成者向け注記を省く「配布用」モード
  (excelExport.ts:180-191 が常時連結)
