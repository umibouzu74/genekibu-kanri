...
を新設**し、小粒バッチ **L1 (8 件) を実装** (テスト 1879 件)。続けて第 2 弾
として **L3a/L3b (講師別上限) / L5a (親マスタ取込) / L3e (部分解内訳) /
L2a (講師ハイライト) / L2c (全日適用・日列コピー) を実装** (テスト 1936 件)。
残る未着手候補は §L 参照。それ以前 (同日): §G.6 推奨順の小粒バッチ ×2 完了
...
3. ~~コードの軽い一手なら E3c か E4b~~ ✅ 両方完了 (2026-07-03。E1h 主要分・
   ...
   **新規の修正・改善候補は §K (2026-07-03 棚卸し) と §L (同日、新機能・
   便利機能レビュー) に集約** — K2a は修正済み。§L は L1 (小粒 8 件) に
   加えて L3a/L3b/L3e/L5a/L2a/L2c も実装済み。残る候補は
   L2b/L2d/L2e/L2f (編集系) · L3c/L3d/L3f (ソルバ系) · L4 系 (入力) ·
   L5b/L5c/L5d (出力・連携) と、仕様判断待ちの L6a
