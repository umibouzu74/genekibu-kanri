...
- ✅ **S2 (2026-07-28 実装)**: 確認テストの回数も S1 のまとめに載せたい —
  ...
  **実装**: クラス別回数シートの各セルに、うち確認テスト付き (= その日の
  ...
  テ表記に更新。
- ✅ **S3 (2026-07-28 実装)**: 個人シート (各講師のシート) にも自分の分の
  クラス別回数を載せたい — 印刷して個人に渡った紙 1 枚で座席表・確認
  テストの必要枚数が分かるように (excelExport.ts)。
  **実装**: 各個人シートのタイトル直下 (行 2) に 1 行まとめ
  「クラス別回数: ３S 5(テ2)、３A 3 ／ 計 8(テ2) ※(テN)=うち確認テスト
  付き(最終コマ)」を全列結合で追加 (凡例はテ表記がある場合のみ)。集計は
  S1/S2 の `computeTeacherClassCounts` を 1 回だけ計算して共用し、行の
  テキストは純粋関数 `buildTeacherCountLineText` に切り出し。タブが複数
  あるときはクラス名にタブ名を前置して同名クラスを区別する。ヘッダ以降が
  1 行下がるため、オートフィルタ (行 3)・日付区切り太線・Print_Titles
  (1:3、まとめ行も毎ページ載る) の行参照を更新。テスト +3、個人シートの
  行オフセット参照を全更新。
