 ...
 * @param {unknown} value 文字列ならそのまま、それ以外は JSON.stringify する
 * @returns {number} 概算バイト数 (直列化不能なら 0)
 */
 ...
 * @param {unknown} project 対象プロジェクト
 * @param {{ limitBytes?: number, warnRatio?: number }} [options]
 * @returns {{ bytes: number, ratio: number, warn: boolean }}
 */
export function checkStorageHealth(project, { limitBytes = STORAGE_LIMIT_BYTES, warnRatio = STORAGE_WARN_RATIO } = {}) {
