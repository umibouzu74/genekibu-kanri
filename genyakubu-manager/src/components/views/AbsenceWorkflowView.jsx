...
export function AbsenceWorkflowView({
  ...
  // 一覧画面から日付指定で遷移してきた場合、ドラフトを破棄して該当日へ移る。
  // (initDate は親が再利用しないよう onConsumeInitDate で消費通知する)
  useEffect(() => {
    if (!initDate) return;
    setDate(initDate);
    draft.reset();
    onConsumeInitDate?.();
    // draft / onConsumeInitDate は ref 安定でないため依存に含めない (初回 mount + initDate 変化時のみ実行)。
