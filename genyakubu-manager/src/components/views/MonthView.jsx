...
export function MonthView({
  ...
  // 日曜に中身があったかで列幅を決める。
  let sundayBusy = false;
  const cellEls = cells.map((d, i) => {
    if (!d)
      return (
    ...
    // 日曜にこの講師の予定 (カード・イベント・テスト期間・部分休講・
    // 日単位の休み) が 1 つでもあれば日曜列を細くしない (下の gridColumns)。
    // 全日休講・未確定の注記だけの日曜は細い列に収まるので数えない。
    if (
      dow === 0 &&
      (dayCards.length > 0 ||
        hasEvent ||
        hasExam ||
        granularHols.length > 0 ||
        offDepts.length > 0 ||
        dayOff.off)
    ) {
      sundayBusy = true;
