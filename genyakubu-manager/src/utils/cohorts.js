...
//           (例: "高松西 数学" → 学校 = "高松西")。
//           高1・2 は「学校の英数 (週2 で通う中核)」のみを 1 コース (セット)
//           として束ね、英数以外 (理科・古文漢文等) はコホート化しない
//           (= グループ終了日にフォールバック)。高3 は科目別 (subj トークン別)。
...
// そのまま埋め込む:
//   - 高校(高1・2): `H|<学年>|<学校>|英数`  (英数だけを束ねるスコープ付き)
//   - 高校(その他): `H|<学年>|<学校>`       (学校 = subj 先頭トークン)
...
// 「学校の英数だけをセットにする」スコープを付ける高校学年 (高1・2)。
const EISUU_SCOPE = "英数";
const isEisuuScopedGrade = (grade) => grade === "高1" || grade === "高2";
...
// 高校 subj の「学校トークンを除いた科目名」が英語・数学系か。
// 例: "高松西 英語"→true, "高松一 文系数学"→true (数を含む),
//     "高松一 物理"→false, "古文漢文"→false (学校トークンのみ)。
// 学校名 (高松西 等) に英/数 は含まれないため、トークン以降だけを見る。
...
export function deriveCohortsFromSlots(slots) {
  ...
  for (const s of slots) {
    ...
    if (dept === "高校部") {
      ...
      const eisuu = isEisuuScopedGrade(s.grade);
      // 高1・2 は英数のみをセット化。英数以外はコホートを作らない。
      if (eisuu && !isEnglishOrMathSubject(s.subj)) continue;
      const id = eisuu ? `H|${s.grade}|${school}|${EISUU_SCOPE}` : `H|${s.grade}|${school}`;
      let c = highById.get(id);
      if (!c) {
        const base = school ? `${s.grade} ${school}` : s.grade;
        c = {
          id,
          dept,
          grade: s.grade,
          school,
          days: null,
          label: eisuu ? `${base} ${EISUU_SCOPE}` : base,
          slotIds: [],
          slotCount: 0,
        };
        highById.set(id, c);
...
//   - 高校 `H|学年|学校[|英数]` : 学年一致かつ subj 先頭トークン == 学校。
//     `英数` スコープ付き (高1・2) は科目が英数のときのみ一致。
//   - 中学 `M|学年|曜日列`      : 学年一致かつ曜日列に slot.day が含まれる
//     (曜日は 1 文字なので includes で判定できる)。
// 導出側がコースをどう束ねても (火金 / 月木 等)、保存済み ID だけで判定できる
// ため全 slot を渡す必要がない (isSlotBeyondCutoff は slot 単位)。
...
// コホート ID (`<種別>|<学年>|<キー>[|<スコープ>]`) が slot に一致するか。
// 学校名・曜日列・学年に "|" は含まれないため "|" で分割する。
function cohortIdMatchesSlot(id, slot) {
  const parts = id.split("|");
  if (parts.length < 3) return false;
  const [kind, grade, key, scope] = parts;
  if (grade !== slot.grade) return false;
  if (kind === "M") return !!slot.day && key.includes(slot.day);
  if (kind === "H") {
    if (firstSubjToken(slot.subj) !== key) return false;
    return scope === EISUU_SCOPE ? isEnglishOrMathSubject(slot.subj) : true;
  }
  return false;
}
