...
### Fixed / Changed / Added (目に見える改善 第 2 弾: 不具合 4 件・使い勝手 4 件・表示 4 件、2026-09-26)

**不具合**

- 玉突き代行の「一括保存」で、欠勤登録時の理由メモ (体調不良など) が
  「玉突き代行」に上書きされていた。既存レコードのメモを引き継ぐ (代行モードと
  同じ)。メモが空のときだけ「玉突き代行」の目印を入れる
- 授業管理 → 代行一覧の「講師・代行者」フィルタで、"香川·福江·川井" が 1 つの
  選択肢として並び、選ぶと 0 件だった (多担任のコマにしか居ない講師は選べ
  なかった)。1 人ずつに分け、隔週のパートナーと時間割に居ない代行者 (直接入力)
  も拾う。教科グループ内はよみ順
- 閲覧者 (講師) のサイドバー・Cmd+K・`g a` に「欠勤組み換え」が出て、開くと
  「管理者のみ」の行き止まりだった。管理者だけに出す (`adminOnly`)
- 講師別月間カレンダーがスマホ幅で 700px 近くまで横にはみ出していた。列を
  `minmax(0,1fr)` にして画面に収め、狭い画面ではカードを折り返す

**使い勝手**

- 講師検索を**よみ**でも当てる (「ほり」で 堀上)。サイドバー・Cmd+K・
  講師比較・欠勤組み換え・複数日の欠勤登録 (`teacherKana.teacherMatchesQuery`)
- 欠勤組み換えのコマを**クリック / タップ / Enter**でも操作メニューを開く。
  右クリックだけだと iPad では代行・振替・合同を登録できなかった。合同の相手を
  選んでいる間は従来どおり相手選び。コマ休講のカードもクリックで取り消せる
- 欠勤組み換えの日付送りで日曜を飛ばし、← / → / t キーも効かせる
  (ダッシュボード・代行モードと同じ。下書きガードは通す)
- 休講・テスト期間・特別イベント・追加授業・特別時程の一覧の ✏️ で、編集
  フォームまでスクロールする (`useEditTarget.scrollFormIntoView`)。画面外の
  フォームだけが書き換わり、押しても何も起きないように見えていた

**表示**

- **授業管理の一覧に期間** (代行一覧・時間割調整一覧・回数補正一覧)。「月」の
  入力を休講などの一覧と同じ「今月以降 / 月を指定 / すべて」にした
  (`ListPeriodSelect`)。既定は今月以降 (以前は当月固定で、月末に来月の代行が
  見えず、「すべて」は月の入力を空にするという見えない操作だった)。期間の外の
  日付で登録したらその月の指定へ動かす。時間割調整一覧・回数補正一覧は一度
  開いたら隠して残し、タブを切り替えても絞り込みと並び順が消えない
- **イベントカレンダーに対象と時間割調整**。休講・テスト期間のチップに
  「中3」「高校部」「英語」などの対象を小さく添え、ツールチップと月の一覧には
  「対象: 中学部 中3」と全文で出す (`utils/eventTargets`。休講日の注意書きと
  共有)。日まるごと振替を振替元 (「↻ 振替 N コマ → M/D」) と振替先
  (「← M/D」) の両方に、コマ休講を「🚫 コマ休講 N」で出す
  (`utils/eventCalendarAdjustments`。集計は `collectIncomingReschedules` /
  `collectOutgoingReschedules` / `collectCancelledSlots` に委ねる)。クリックで
  その日のダッシュボード。月グリッドの列を等幅にした
- **ヘッダの時間割セレクタに今日の状態**。選択肢に「(今日有効)」「(終了)」
  「(開始前 M/D〜)」を付け、表示中の時間割が今日有効でなく他に有効な時間割が
  あるときは「⚠ 表示中の「1学期」は 8/31 で終了しています」+ 切り替えボタンを
  出す (自動では切り替えない・紙面には出ない)。時間割を選んだことのない端末は
  id=1 ではなく今日有効な時間割を表示する (期切替にも追従)。選んでいた時間割が
  削除されていたら今日有効な時間割に戻る (`utils/timetable` の
  `resolveActiveTimetableId` ほか)
- **講師の iCal 書き出しに終わりと抜け**。時間割の終了日と表示期間の終了日
  (コース別終講日 → 学年グループ) の早い方で繰り返しを止め (UNTIL。実際の
  最終授業日まで詰める)、休講・授業を止めるテスト期間・コマ休講・特別時程の
  部分休講の日を EXDATE で抜く。判定は第N回と同じ述語 (`isSlotHeldOnDate` +
  `isSlotBeyondCutoff`、隔週の週は `isTeacherActiveOnDate`)。終了日が無ければ
  従来どおり (EXDATE は 1 年先まで)。期が終わって出す授業が無いときは
  toast で知らせる。表示期間の窓は `timetable.getSlotCutoffRange` に切り出し
- テスト: `TimetableSelector.test.jsx` / `App.timetableSelection.test.jsx` /
  `eventTargets.test.js` / `eventCalendarAdjustments.test.js` (新規) ほか
  `ics.test.js` (+21)・`EventCalendarView.test.jsx` (+8)・
  `SubstituteView.test.jsx`・`AbsenceWorkflowView.test.jsx`・
  `ChainSubstitutionPanel.test.jsx`・`Sidebar.test.jsx`・
  `CommandPalette.test.jsx`・`HolidayManager.test.jsx` など

### Changed / Added (目に見える改善: 月間・バイトの出勤・一覧の曜日・代行の連絡文、2026-09-26)

- **講師別の月間カレンダー** (`MonthView`)
  - その月の日曜にこの講師の予定 (カード・イベント・テスト期間・部分休講・
    日単位の休み) が 1 つも無いときは、日曜列を細くして平日の列に幅を回す
    (紙面も同じ列幅)。全日休講・未確定の注記だけの日曜は細いまま
  - 過ぎた日を薄く出して今日以降を目立たせる (`.month-past`、画面だけ。
    マウスを乗せる / フォーカスで元の濃さ。印刷には効かない)
- **バイト管理の出勤状況を月のミニカレンダーに**
  (`staff/StaffAttendanceCalendar`)。「2026-09-01 (火)、2026-09-03 (木)、…」
  の羅列をやめ、通常出勤 (青) / 代行出勤 (緑) / 代行された日 (赤・取消線) /
  欠勤・代行未定 (赤の点線枠) で日ごとに塗る。重なる日は強い方で塗り、残りを
  セル下の点で出す。件数は凡例に、従来の日付の羅列は「日付を一覧で見る」で開ける
- **よみ欄の例示「ほりかみ」をやめる** (`KanaField`)。全員の欄に同じ例が出て
  いたため、奥村の欄に「ほりかみ」が入力済みのように読めた。「未設定 (ひらがな)」
- **一覧の日付に曜日**。休講一覧・テスト期間 (期間と例外日)・特別イベント・
  イベントカレンダーの月の一覧 (`formatDateRange(..., { weekday: true })`)
- **代行一覧に 💬「連絡文をコピー」** (`utils/subContactMessage`、管理者のみ)。
  LINE などにそのまま貼れる文面を 4 状態で出し分ける (代行未定 = 探している /
  依頼中 = 宛先の先生に頼む / 確定 / 代行なし)。同じ日・同じ元講師・同じ代行者・
  同じ状態の行は時刻順の箇条書きで 1 通にまとめる。時刻は**その日の実際の
  時刻** (コマ移動 > 特別時程 > 元の時刻)。欠勤理由のメモは載せない
- CLAUDE.md の却下リストに「ダッシュボード時間割の横スクロール解消」と
  「いま授業中の強調」を追加
- テスト: `StaffAttendanceCalendar.test.jsx` (新規 5)、
  `subContactMessage.test.js` (新規 7)、`MonthView.test.jsx` (+4)、
  `SubListTab.test.jsx` (+2)、`dateHelpers.test.js` (+1)

### Fixed / Changed (多担任コマの欠勤表示、2026-09-25)

- タイムテーブル / ダッシュボードの時間割モード (`ExcelCell`) で、多担任
  コマ (プレップ「香川·福江·川井」等) の 1 人だけが休むと、出勤する人の
  名前にも取消線が通って全員休みに見えていた不具合を修正。取消線を親の
  div に付けていたため、子の `textDecoration: none` では消せなかった
  (CSS の text-decoration は子へ伝播する)。休む人の名前だけに線を引き、
  出勤する人はそのまま (名前から月間へ飛べる)
- 同じコマの欠勤の行に**誰が休むか**を添える (「代行なし」→
  「香川 休み（代行なし）」、代行未定も同様)。読み上げ名も講師ごとに
- 「代行なしで確定」だけのコマは赤で塗らず、状態の色 (茶) にする。
  赤はまだ手が要る「代行未定」のときだけ
- 代行モードで欠勤に選んだ人も、多担任コマではその人だけに取消線

### Fixed / Changed (特訓シフトの入力まわり、2026-09-25)

- モーダルで操作するたびにスクロールが先頭へ戻る不具合を修正
  (`hooks/useFocusTrap`)。`onClose` を effect の deps に入れていたため、
  呼び出し側がインライン関数を渡すモーダル (特訓シフト等) では保存 → 再描画の
  たびに trap が作り直され、裏のボタン → 右上の ✕ へフォーカスが移っていた。
  講習作成側 (`timetable-builder/hooks/useFocusTrap.ts`) と同じく ref で持つ。
  本体の `useFocusTrap` を使うもの (`Modal` を使う画面すべて・コマンド
  パレット・通常時間割作成の ⚙ 全体設定) に効く
- 特訓シフトの出勤表:
  - 見出しに校時の時刻と出勤人数を出す。広い画面では、下へスクロール
    しても見出しと日付リストがモーダル上端に残る (狭い画面は固定せず、
    従来どおり表だけを横スクロール)
  - セル全体がクリック範囲。チェックしたセルは色付き、出勤のある講師は太字
  - 「全て」のボタンを状態に応じて「全選択」/「全解除」に
  - 絞り込みを**よみ**でも当てる (`teacherKana.teacherMatchesQuery`)。
    「出勤者のみ」で割り当て済みの講師だけに絞れる。最後のチェックを
    外しても、その日を開いている間は行を消さない (誤って外してもその場で
    入れ直せる)
  - 日付リストに出勤人数 (「● 5人」)
  - 候補 (バイト + 全時間割のコマの講師) に居ないのに出勤が残っている講師
    (コマの削除・置き換え反映・講師名の書き換えで時間割から居なくなった人)
    を「候補外」の行で出す。これまでは表に出ないので外せず、別の日への
    コピーにも黙って含まれていた
  - コピー先に選んだ日を開くと「N 日にコピー」がその日まで数えていたのを
    修正 (表示中の日は数えない。選択は残すので、戻れば元どおり)

### Fixed / Changed (branch 全体の校正: 正しさ 10 件 + 使い勝手 20 件、2026-09-19)

第 1・2 弾で入れたものを main 比でまとめて校正し、指摘を全部反映した。

**正しさ**

- 代行モード: 多担任コマの玉突き提案を (コマ × 講師) で引くよう修正
  (`useSubstitutionMode.getSuggestion`。2 人目の提案が 1 人目を上書きし、
  ポップオーバーが別人の提案を採用していた)。「そのコマのその日の欠勤者」の
  判定は `absenceHelpers.absentTeachersForSlot` に集約 (セルのクリック先と
  ポップオーバーの担当切替で同じ関数)
- 週間ビュー: 日曜に開くと「今週」が点かず t / 今週 が効かないように見えて
  いた (`weekMondayOf` が日曜を翌週に寄せるため)。判定を
  `weekMondayOf(today) === weekMonday` にし、「(日曜のため翌週を表示)」を添える。
  お知らせの見出しは実際の窓 (「12/8〜12/22 のイベント:」) で書く
  (週を送っても「直近2週間」のままだった)
- イベントカレンダー: 表示月を今日からの差ではなく絶対値 ("YYYY-MM") で
  持つ。月末に開きっぱなしのタブが翌 0 時に勝手に翌月へ進み、保存値まで
  書き換えていた
- 授業管理の月フィルタ追従: subs の増減を見張る方式をやめ、保存経路
  (`useSubsCrud.save` の戻り値 = いま作ったレコード) を App が `createdSubs`
  で渡す。フォームを開いている間に他端末の同期で増えた分で月が飛んでいた
- 1日分まとめて代行: 日付を変えると共通メモも空に戻す (前の日の理由が別の
  日の全件に付いていた)
- 特別時程の同日重複判定 (`dayScheduleTimeKeys`): 読み替え先が空 / 元と同じ
  で実際には効かない timeMap の行を「その時間帯を取っている」と数えていた。
  `resolveSlotDaySchedule` と同じ述語に
- 教科マスター: カテゴリ色をドラッグした直後にそのカテゴリを削除すると、
  アンマウント時の未確定色の書き戻しが削除前の一覧を握った保存関数で走り、
  消したカテゴリが復活していた。確定は常に今の一覧を ref で見て id が
  無ければ捨てる
- 整理: サイドバーの「未処理の代行」ジャンプを `selectView(v, before)` に
  (ナビの手書き重複)、M/D 表記を `dateHelpers.fmtMD` に集約

**使い勝手**

- 「欠勤組み換えへ」の導線を 🚑 + 「YYYY-MM-DD (曜) の欠勤組み換えを開く」
  (読み上げ名 = title) に統一 (代行一覧 / 調整一覧 / 回数補正一覧 /
  カレンダーの日付 / 日別ダッシュボード。旧「欠勤振替画面」を一掃)
- 月送りを共通部品 `components/MonthNav` に (講師別月間とイベントカレンダー
  で読み上げ名・title・「今月」の強調・aria-live・no-print が揃った。App の
  月ナビは `<input type="month">` が紙に出ていた)
- 日付入力に見える見出しをラベル付け (週間「表示する週」/ ダッシュボード
  「表示日」)。範囲表示に aria-live (キーで動いたことが読み上げられる)
- ← / → は radio・listbox・tab・grid など矢印キーを自前で使う部品に
  フォーカスがある間は横取りしない (`keyboardGuards.hasArrowKeyOwnerFocus`)。
  時間割の代行モードの日付にも ← / → / t を配線 (日曜は飛ばす)。
  ショートカットヘルプの注記を更新
- カレンダーの 🚑 は薄く出してホバー / フォーカスで濃く、ヘッダに凡例
  「日付クリック = その日のダッシュボード / 🚑 = 欠勤組み換え」。紙面では
  日付の点線下線を消す。モバイルの 40px 最小高の規則から行内ボタン
  (日付・🚑・追加授業バナー) を除外 (`inline-activate`)
- 代行一覧: 対象日 / 作成日時の両列で昇順・降順 (aria-sort 付きボタン)。
  作成日時列は紙に載せない
- 代行ポップオーバーの担当切替は折り返し、欠勤者 4 人以上は select
- 1日分まとめて代行の空表示を「登録済みを除く」に直し、「登録済み: 代行あり
  N 件 / 代行未定 M 件 / 代行なし K 件」の内訳を出す
- 特別時程: 同じ日・学年・時間帯の既存があるときは押してからではなく編集中
  から赤い注意を出し、登録 / 更新ボタンを無効にする (「既存を編集」へ誘導)
- 時間割管理: 入力のたびに再評価される重なり警告を role="status" に
  (alert は保存後の止めるエラーだけ)。エラーが無いときは aria-describedby を
  付けない。重なりの区間は年が今年と違う / 年をまたぐとき「2027/4/1」の
  ように年を付ける。学年の区切りを「・」に統一
- 下書きガードの文言を `utils/absenceDraftPrompt` に統一 (「下書きが N 件
  あります。破棄して 9/21 (月) へ移動しますか？」)。App へは下書きの件数を
  渡す。欠勤組み換えの「🔗 玉突き代行で探す」は下書きを捨てさせず
  「保存して開く」(キャンセルなら留まる)
- テスト +44 (`MonthNav.test.jsx` / `DayNumberLink.test.jsx` /
  `keyboardGuards.test.js` / `SubstitutionPopover.test.jsx` 新規ほか)
...
(2026-09-15 の点検、43 件の候補から「正しい動きが既に決まっているもの」を
第 1 弾として実装)。

- **代行モード (タイムテーブル) の仮代行を「コマ × 講師」単位にした**
  (`useSubstitutionMode` の `pendingKey`)。プレップのような多担任コマで
  2 人目の仮代行を入れると 1 人目が黙って消えていた。セルのクリックはまだ
  代行の付いていない欠勤者を先に出し、セルには人数分の仮代行を並べる
  (`ExcelCell` の `pendingSubs`)。2 人とも付いた後はポップオーバーの
  「担当: 香川 [→ 福江]」で切り替えて直せる (`SubstitutionPopover` の
  `absentTeachers` / `onSwitchTeacher`)
- **代行モードの「確定して保存」で保存件数を toast で出す**。欠勤登録時の
  理由メモは代行者を決めても消えない (同じ (日付, コマ, 元講師) の
  レコードを置き換えるときに `memo` を引き継ぐ)。「破棄」の確認を他画面と
  同じ `useConfirm` にした (`window.confirm` をやめた)
- **授業管理 → 時間割表 (代行モード) に特別時程を配線** (`SubstituteView` →
  `ExcelGridView` の `daySchedules` / `teacherKana`)。附属の 1限カット /
  50 分授業の日でも 移・休 と「⏰ 特別時程」バナーが出る (以前はダッシュ
  ボードだけ)。1限カットのコマは代行モードでも休講扱い (代行対象に出ない)
- **授業管理の ＋ 新規代行で来月の代行を登録すると一覧から消えていた**
  (当月の月フィルタ)。登録後は月フィルタをその月へ動かす (「すべて」は
  動かさない。他端末からの同期では動かない)
- **1日分まとめて代行: 多担任コマは 1 人に代行記録が付いても行を消さない**。
  元講師の選択肢を未登録の人だけに絞る (「(他 N 名は登録済)」)。判定は
  単一コマフォームと共有の `components/substitute/slotTaken.js`
  (`getSubForSlot` の先頭 1 件で絞っていたため行ごと消えていた)。
  行ごとに**「代行未定で登録」** (代行者空 + 依頼中 = 欠勤・代行未定) を
  選べるようにし、その日の全件に共通のメモ欄を足した。「明日は全部休み、
  代行はこれから探す」をここから登録できる
- **玉突き代行の「空き講師」に、その日に担当コマが無い講師も出す**
  (`chainSubstitution.computeAvailableTeachers` の `includeIdleTeachers`、
  理由「この日は担当なし」)。候補を「その日のコマの講師」で閉じない
  (常勤はその曜日にコマが無くても代行に入る)。その日に既に引き受けている
  代行の時刻は塞がった扱い。タイムテーブル代行モードの「空き / 授業中 /
  時間重複」は従来どおり (オプトインは玉突き代行の画面だけ)。空き講師と
  同点候補の並びは関連度 → よみ順、手動追加は一覧に無い名前も直接入力できる
- **日別ダッシュボードで隔週コマの A/B 週を日付で解決する** (`SectionColumn`)。
  時間割モードと同じ A週/B週バッジ + その週の担当者 + 実施教科。B 週は
  パートナーの名前を出し、名前クリックで月間へ飛ぶ導線もパートナーで動く。
  アンカー未設定で週が決まらないときは従来どおり併記
- **週間ビューの「直近2週間のイベント」に休講を 🚫 で出す**。曜日マスは
  日付を持たないので、これまで週間ビューのどこにも休講が出ていなかった
  (コード内コメントは逆のことを主張していた)。対象は自分のコマに当たる
  休講だけ。部門・学年・教科の絞り込みはダッシュボードと同じ判定
  (`makeEventHelpers.isHolidayForSlot`)、隔週コマはその日の担当週だけ見る
- **イベントカレンダーの「今日」の強調が、タブを開いたまま日付を跨いでも
  翌 0 時に更新される** (他ビューと同じ `useToday`)
- **ダッシュボードの追加授業バナーの行をクリック / Enter / Space で編集へ**
  (日別・時間割モードの両方。月次・週間・イベントカレンダーと同じ導線)
- **教科マスターのカテゴリ名・教科名・別名は blur / Enter で 1 回だけ保存**
...
- テスト: `useSubstitutionMode.test.jsx` (新規)、`SubstituteView.test.jsx`
  ...
  (新規)、`ExcelGridView.test.jsx` / `DashDayRow.test.jsx` /
...
- **休講日の編集で日付を直せなかった** (同日に入れた連続日登録の副作用)。
  編集中はチップではなく日付欄を 1 つ出し、日付そのものも直せるようにした
  (`HolidayManager`)
- 複数日の欠勤登録の「対象外」判定が講師欄を手書きで分解していたのを
  `biweekly.isSlotForTeacher` に寄せた (CLAUDE.md「複数講師の区切り文字」)
- ダッシュボードの表示日の復元は「その日に保存したもの」だけ。開きっぱなしの
  タブを翌朝リロードしても昨日の日付から始まらない (`Dashboard.jsx`)
- **月次の 📋 まとめて印刷でスナップショット中に例外が出ると、popup が
  「準備中」のまま残り toast も出なかった** (今回より前からの不具合)。
  全曜日印刷と同じく popup を閉じてエラー toast を出す (`usePrintJobs`)


### Added (日別ダッシュボードにも講師重複 / 講師名クリックで月間へ / 欠勤組み換えが特別時程と追加授業を見る)

- **日別ダッシュボードにも講師の同時刻の重なり ⚠ を出す** (`DashDayRow` →
  `SectionColumn`)。時間割モードにしか無く、同じ日を日別で見ると警告が
  消えていた。判定は `utils/teacherConflicts` の同じ関数、索引は
  セクション横断 (中学部 ↔ 高校部) で日単位に 1 回だけ組む。カード内に
  「⚠ 福江: 中3A 理科 (代行) と重複」の行
- **講師名をクリックするとその人の月間スケジュールへ**。日別ダッシュボードの
  カード (`SectionColumn`)・時間割のセル (`ExcelCell`、閲覧モードだけ。
  代行モードはセル全体がクリック対象)・講師比較の列見出し (`CompareView`)。
  "香川·福江" のような複数講師は 1 人ずつ。これまでサイドバーを目で
  スクロールして探すしかなかった
- **欠勤組み換えが特別時程と追加授業を見る** (`AbsenceWorkflowView`)。
  特別時程で休講 (1 限カット) のコマは欠勤対象に出さず、時刻の読み替えが
  ある日は上にバナーで知らせる (グリッドの時刻は元の時刻のまま)。追加授業は
  その日の担当を確かめる材料としてバナーで出す (代行はここでは登録しない)
- **代行一覧に「📥 表示中を CSV」** (`SubstituteView`)。月・講師・状態で
  絞った結果だけを CSV にする (全件はデータ管理から)
- 全講師一覧の同点はよみ順で割る (`AllView`)。同じコマ数の講師が登録順に
  並んで毎回順番が変わって見えていた。主たる並び (コマ数の多い順) は変えない
- テスト: `DashDayRow.test.jsx` (+3)


### Added (登録の手数を減らす: 連続日の休講・一覧の期間絞り込み・カレンダーから日付つき登録・Cmd+K の日付ジャンプ)

- **休講日を連続日でまとめて登録** (`HolidayManager`)。年末年始・盆休みは
  「開始日 〜 終了日 → ＋ 日付を追加」でチップに足し、同じ対象設定で一括
  登録する (追加授業と同じ作法。id は連番)。日付欄に入れたまま「追加」を
  押しても登録できる。編集は 1 件だけ
- **休講・テスト期間・特別イベント・追加授業・特別時程の一覧に期間絞り込み**
  (`components/ListPeriodFilter` + `hooks/useListPeriod` +
  `utils/listPeriod`)。既定は「今月以降」(今月に入ってから過ぎた分も残す)、
  「月を指定」「すべて」。5 つのマネージャは全件をベタ表示していたので、
  運用 1 年で去年の分が先頭を占めていた。絞り込みは表示だけで、編集・削除・
  カレンダーからのジャンプは全件が対象。絞り込みで 0 件のときは「登録なし」
  と言わず「この期間に該当なし」と出す
- **イベントカレンダーの日付セルから登録** (`EventCalendarView`)。各日の
  「＋」→ 種別 (休講 / テスト期間 / 特別イベント / 追加授業 / 特別時程) を
  選ぶと、その日付が入った登録フォームが開く (`useNewEntryTarget` の
  `date` → 各マネージャの `resetForm(presetDate)`)。「10/13 を休講に」で
  カレンダーを見て別画面で日付を打ち直す往復をなくす
- **Cmd+K に日付ジャンプと空のときの一覧** (`CommandPalette`、
  `utils/parseDateQuery`)。「9/24」「2026-09-24」「0924」でその日の
  ダッシュボード / 欠勤組み換え (管理者のみ) を開く。年の無い書き方は今年、
  3 か月より前になる日付は来年に読む。空のときは今日・明日の日付ジャンプと
  主要なビュー・操作を**固定の**一覧で出す (使用頻度や履歴で並べ替えは
  しない)。複数講師のコマ ("香川·福江") のヒットは講師ごとに 1 件にした
  (結合文字列をそのまま講師名として選んでいた)
- **欠勤登録ダイアログに理由メモ** (`AbsenceRegisterDialog`)。選んだ全件に
  同じメモを入れる。保存後に授業管理で 1 件ずつ ✏️ を開き直さなくてよい
- **バイト管理の出勤状況に月セレクタ** (`StaffManagerView`)。今月固定
  だったので、月初に前月ぶんを確かめられなかった
- ショートカット一覧に `g b` (講習時間割作成) / `g r` (通常時間割作成) を
  載せた (定義はあったが一覧に無かった)
- テスト: `HolidayManager.test.jsx` (新規 7)、`listPeriod.test.js` (新規 4)、
  `parseDateQuery.test.js` (新規 3)、`CommandPalette.test.jsx` (新規 4)、
  `EventCalendarView.test.jsx` (+2)、`AbsenceWorkflowView.test.jsx` (+1)


### Added (ダッシュボードの「要対応」を両モードに出し、そこから 1〜2 手で片付ける)

「今日なにが要対応か」がダッシュボードの既定表示 (時間割モード) には一度も
出ず、代行未定の一覧もサイドバーの赤バッジも実態を表していなかった
(2026-09-12 の点検)。

- **要対応カードを表示モードを問わず出す**。代行サマリ (今日 / 明日の代行)
  と直近 7 日のイベントは日別モードの中にしか無く、既定 (時間割) のままの
  人には永久に見えなかった。日付ナビの直下へ移動 (`Dashboard.jsx`)
- **「代行未定 (探し中)」と「依頼中 (未確定)」をカードで分け、今日以降だけ
  数える** (`SubSummaryCards`)。過去の未処理は「過去の未処理 (確定し忘れ?)」
  として小さく別枠。どのカードもクリックで代行一覧をその絞り込みで開く。
  今日 / 明日のカードは管理者ならその日の欠勤組み換えへ飛ぶ
- **サイドバーの赤バッジは「今日以降の未処理」だけ** (`Sidebar.jsx`)。
  全期間の requested を数えていたので前期の確定し忘れで永久に赤いままだった。
  加えて、トップレベル項目 (授業管理) にバッジを描く分岐が無く、そもそも
  どこにも出ていなかったのを直した
- **代行一覧のステータス絞り込みを 4 状態 + 「未処理 (未定 + 依頼中)」に**
  (`SubListTab`、`utils/substituteState.SUB_STATE_FILTERS` /
  `matchesSubStateFilter`)。旧 2 値 (依頼中 / 確定) では「代行未定」で
  絞れなかった。未処理の行は左に色帯 (未定 = 橙 / 依頼中 = 赤)、
  「未処理」で絞ると今日以降 / 過去の件数を出す
- **代行一覧の行内で「代行者名 + ✓ 確定」** (`SubListTab` の
  `QuickResolveCell`、`useSubsCrud.quickUpdate`)。未処理の行だけに出し、
  モーダルを開かずに名前を入れて確定できる。Enter は名前だけ保存 (未定 →
  依頼中)、✓ 確定 は名前ごと confirmed (空欄なら「代行なしで確定」)。
  フォーカス外れでは保存しない (blur で保存すると ✓ 確定 のクリックが
  行の差し替えで届かない)
- **ダッシュボードから欠勤組み換えへ**。日付ナビの「🚑 この日の欠勤組み換え」
  と、日別モードの日付帯の「🚑 欠勤組み換え」(管理者のみ)。
  `App.jumpToAbsenceFlow` は実装済みで授業管理にしか配線されていなかった
- **欠勤組み換えに「← 前 / 今日 / 次 →」**と、欠勤する先生の一覧を
  「この日に担当あり / その他」に分け、名前で絞り込めるようにした
  (`AbsenceWorkflowView`)。担当の有無は `absenceHelpers.activeTeachersOnDate`
  (隔週 A/B 解決後) で判定。誰でも選べることは変えない
- 日別モードの日付帯に「今日」チップ

### Fixed (「今日」が更新されない / 日曜に進むと表示がずれる / 表示日が戻る)

- **タブを開いたまま日付を跨いでも「今日」が動かない**画面を `useToday` に
  揃えた (ダッシュボード / 週間 / 月間 / 代行確定一覧 / 授業管理 / サイドバー)。
  事務室の PC は開きっぱなしなので、翌朝の「今日の代行」が前日のままだった
- **時間割モードで土曜から「次 →」で日曜へ進むと、表示日と曜日タブが別の
  週にずれる** (`ExcelGridView` は日曜を表せず「今日基準の直近の土曜」へ
  落ちていた)。「← 前 / 次 →」は日曜を飛ばし (`DashboardDateNav.stepDate`)、
  日付欄で日曜を選んだときは「日曜日です」のバナーと土曜 / 月曜へのボタンを
  出す (黙って別の日を描かない)
- **ダッシュボードの表示日が、別の画面へ寄って戻ると今日に戻る**。タブ単位
  (sessionStorage) で覚えるようにした。タブを閉じれば今日に戻る
- テスト: `substituteState.test.js` (+7)、`SubListTab.test.jsx` (+5)、
  `SubSummaryCards.test.jsx` (新規 3)、`DashboardDateNav.test.jsx` (新規 3)、
  `Sidebar.test.jsx` (+2)、`AbsenceWorkflowView.test.jsx` (+2)
...
ような早い校時を足しても前へ動かす手段が無かった (`ExamPrepScheduleEditor`)。

- **校時テーブルからフォーカスが外れた時点で開始時刻順 (同時刻なら終了時刻順)
  に並べ替え、番号を 1 から振り直す**。出勤のチェックは校時に付いて動く
  (旧 no → 新 no の読み替え)。時刻の onChange で並べ替えないのは、入力中に
  行が飛ぶと編集しづらいため。テーブル内の別の欄へ移る間は据え置く
- 並べ替え・削除の連番振り直しと出勤チェックの読み替えは
  `examPrepHelpers.applyPeriodOrder` / `sortDayPeriodsByTime` に集約
- 開始時刻を読めない校時は相対順のまま末尾。並びが変わらなければ保存しない
- ↑↓ での手動入れ替えは付けない (時刻順に自動で揃うので、動かしても次の
  並べ替えで戻ってしまう)

### Added (代行を入れた後の講師の同時刻の重なりを警告)

2026-09-10 (木) の欠勤組み換えで、福江が 19:50 に 中2C 数学 (奥村の代行) と
中3A 理科 (小見山の代行) で二重になっても、本体はどの画面も何も出さなかった。
講師重複の NG 判定は作成ツール (講習時間割作成 / 通常時間割作成) にしか無く、
本体は代行を選ぶ瞬間に経路ごとにバラバラな判定 (欠勤組み換えのピッカーは
判定なし) があるだけで、保存後は誰も見ていなかった。

- **判定を `utils/teacherConflicts.js` に集約**。その日に実際に教える人
  (元講師 − 代行/欠勤で手を離れた人 + 代行者・仮代行・下書き) を集め、
  移動・特別時程を反映した時刻で重なりを探す。休講・テスト期間・振替で出る
  コマ・合同で吸収された側は数えない
- **タイムテーブル (閲覧 / 代行モード) のセル**に「⚠ 重複」バッジと
  「⚠ 福江: 中3A 理科 (代行) と重複」の行を出す。中学部 ↔ 高校部を横断して
  当てる
- **欠勤組み換えのカード**にも同じ表示 (下書きの代行を含めて判定)
- **欠勤組み換えの代行ピッカー**は候補ごとに「授業中: 中2C 数学」
  「代行中: 中3A 理科」を出す。どれも**警告であって禁止ではない**
  (1 人で 2 教室を回す運用があるので、選べなくはしない)

### Fixed (登録した代行がスケジュールに出ない / 常勤を代行者に選べない)

2026-09-10 (木) の中3C 社会 (野口 → 西岡) で、欠勤組み換えでは西岡を代行者に
選べず、授業管理 → 代行登録で登録しても時間割・ダッシュボードのどこにも
出なかった。原因は 2 つ。

- **授業管理 → 代行登録のコマ一覧が曜日だけで絞られていた**
  (`SingleSubForm` / `DayBulkSubForm`)。期切替で残してある旧期の時間割
  (終了日入り) にも同じ木曜 中3C 社会 があり、そちらを選ぶと一覧には載るのに
  スケジュールには出ない (日付ベースの画面はその日に有効な時間割のコマしか
  引かない)。欠勤登録と同じ `absenceHelpers.getAbsenceDaySlots` (時間割の
  有効期間 + 表示期間) で絞るようにした。編集中のレコードが期間外のコマを
  指していれば、その 1 件だけは「⚠ この日は有効期間外」付きで残して
  選び直せる
- **欠勤組み換えの代行ピッカーの候補が「バイト + その日のコマの講師」だけ**
  だった (`SubstitutePickerPopover`)。木曜にコマの無い常勤はどこにも出ない。
  「全員表示」に時間割に出てくる全講師 (曜日を問わない `allTeachers`) を
  含め、それでも無い名前は欄に直接入力できるようにした (授業管理の
  フォームと同じ扱い)
- **代行一覧に点検を足した** (`SubListTab`)。レコードの指すコマがその日に
  有効でなければ「⚠ この日は期間外」バッジと件数を出す
  (`absenceHelpers.isSlotShownOnDate`)。**既に旧期のコマへ登録してしまった
  レコードは自動では直らない** — 一覧の警告から ✏️ で同じ曜日の有効なコマへ
  付け替える
- 代行者の入力候補 (datalist) は講師欄を分解して足す ("香川·福江" を
  1 人として扱っていた)
- テスト: `SubstituteForm` 5 件、`SubstitutePickerPopover` 3 件、
  `SubListTab` 2 件、`isSlotShownOnDate` 2 件
