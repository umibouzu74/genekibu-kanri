...
- 講習時間割作成で組んだ講習コマを、個人の月次カレンダー (MonthView) に
  ...
  年跨ぎも基準日との距離で解決)。正は常に builder 側で親アプリからの
  書き戻しは無く、編集も builder 側で行う (カードにクリック導線なし)。
- 同じ日・時限・講師・科目の複数クラス (合同) は 1 枚のカードにまとめて
  クラスを "/" 連結で表示。講師「未定」のセルは載せない。予備校・高校等の
  externalSessions も反映しない (親アプリに通常コマとして存在する想定の
  重複のため)。時限の時刻はラベル ("1限 (13:00~)") から解析し、読めない
  時限は時限名で表示する。
