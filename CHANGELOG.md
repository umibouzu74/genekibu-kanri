...
### Added (講習時間割作成: 講師別 Excel にタブ別セクション)
- 講師別スケジュールの各講師シートで、従来の統合表 (全タブ + 外部授業) の
  下に**タブ別の抜き出し表 (【中3】【中2】…) を縦に積んで**出力する。
  特定の学年分だけを切り出して確認・配布できる。講師 × タブでシートを
  分けるとシート数が爆発するため、同一シート内の縦積みにした。
- タブが 1 つのプロジェクトは統合表と同一内容になるため従来どおり
  (セクション無し)。担当コマの無いタブのセクションは省略。外部授業は
  どのタブにも属さないため統合表にのみ載る。全講師リストシートは変更なし。
