...
### 通常時間割作成: 🧩 セット編集 (コースセットの曜日を横並びで一画面編集)
- **🧩 セット編集**: 「中3（火・木）」「中3（水・金）」「中1 のセット1/
  セット2」のような、同じクラスが決まった曜日の組で通う**コースセット**を
  1 画面で編集できる表示モードを追加 (📅 週表示の隣のトグル。週表示とは
  排他)。セットのチップ (学年色 + 入力済みコマ数) で切替え、セットの
  曜日の表を横に並べて表示する。
- セットは**スケジュールから自動検出**する (`courseSets.computeCourseSets`。
  設定・保存は不要で、コマを動かせば自動で追従): クラス列の「セルがある
  曜日の組」が同じクラス同士を 1 セットに束ねる。合同列 (範囲・列挙
  ラベル) のセルの曜日は構成クラスの曜日にも数え、合同列自身も構成
  クラスと同じセットへ合流する。セルの無いクラス列は学年の全曜日扱い
  (作りかけの学年もセット編集から入力を始められる)。
- 各曜日は通常の曜日ビューと同じ RegularGrid をセットのクラス列に絞って
  描画する表示専用の変形 (`filterProjectForSet` — splitTabsByCampus と
  同じく tab.id / classId / schedule を保つ) なので、セル編集・右クリック
  メニュー・複数選択・一括操作・ロック・合同の結合表示・重複/NG の赤枠は
  従来のまま機能する。講師プルダウンの「(重複)」「(NG)」予告は絞り込みの
  外のセルも含む全体で計算する (RegularGrid の fullProject prop)。
- **グリッド横断 D&D**: 別の RegularGrid へのドラッグ入替・Ctrl+ドラッグ
  ...
  別プロジェクトを開いた別ウィンドウからの誤ドロップは弾く)。セット編集の
  「火のコマを木へ移す / コピーする」に加え、📅 週表示でも曜日をまたぐ
  入替ができる。
- 重複一覧・Undo toast の「→ 表示」ジャンプは、対象を丸ごと含む別セットが
  あればセットを切替えて点滅表示し、どのセットにも収まらなければ通常の
  曜日表示へ戻してから表示する。🖨 印刷は表示中のセットを曜日ごとに
  改ページして刷る。選択セットはリロード後も復元 (sessionStorage)。
