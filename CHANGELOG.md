...
- モーダルで操作するたびにスクロールが先頭へ戻る不具合を修正
  ...
  本体の `Modal` を使う画面すべてに効く
- 特訓シフトの出勤表:
  - 見出しに校時の時刻と出勤人数を出し、下へスクロールしても見出しと
    日付リストがモーダル上端に残る (狭い画面は従来どおり表だけ横スクロール)
  - セル全体がクリック範囲。チェックしたセルは色付き、出勤のある講師は太字
  - 「全て」のボタンを状態に応じて「全選択」/「全解除」に
  - 絞り込みを**よみ**でも当てる (`teacherKana.teacherMatchesQuery`)。
    「出勤者のみ」で割り当て済みの講師だけに絞れる
  - 日付リストに出勤人数 (「● 5人」)
  - 候補 (バイト + 時間割の講師) に居ないのに出勤が残っている講師
    (期切替でコマが無くなった常勤など) を「候補外」の行で出す。これまでは
    表に出ず外せないのに、講師別の画面には特訓が出続けていた
  - コピー先に選んだ日へ切り替えると「N 日にコピー」の件数だけが
    ずれていたのを修正 (切り替えた日をコピー先から外す)
