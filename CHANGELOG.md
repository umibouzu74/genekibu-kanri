...
- **既定のコース束ね (`utils/cohorts`) は「中学は学年の平日ぜんぶで 1 コース」**。
  中3 のように週内で **火木コース / 水金コース** に分かれる学年は、授業セットを
  登録しないと必ず 4 曜日が 1 本のカウンタになる
- **授業セットはコマ id を直に指していた**ので、期切替で新しい期のコマを作ると
  紐付けが切れる。切り替えるたびに作り直さないと ① の既定へ落ちる

そこで**授業セットの定義を (学年, 曜日) の組 (`units`) に変えた**。
コマ id を参照しないので、期切替で新しいコマを作っても紐付けが切れない
（画面の選択単位はもともと「学年 × 曜日」だったので、保存する形をそれに
合わせただけ）。
...
- 旧形式 (コマ id 直参照) のセットには**「旧形式 (コマID固定)」バッジ**と
  **「曜日ベースに変換」ボタン**を出す。読み取りは従来どおり動く
- **期切替の警告は旧形式のセットだけ**に出るようにした。units 形式のセットは
  作り直し不要なので警告しない
- 時間割の複製でも units 形式のセットは複製しない（複製先のコマにも
