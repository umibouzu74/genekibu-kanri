...
### Added (通常時間割作成: 掲示用「中学生コース時間割」の Excel 出力・取込)

- **「🎒 中学生 掲示用」**ボタンを追加。例年の掲示紙面 (◯◯年度 中学生
  コース時間割) のレイアウトで Excel を書き出す (A3 横 1 枚)。対象は
  **中学の学年だけ** — 学年 (無ければタブ名) に「高」を含むタブは出さない。
  - 紙面の骨格は元 xls の実測どおり: **クラス 1 列 = 方眼 2 列 / 時限 1 行 =
    方眼 4 行 (上 2 行 = 科目・下 2 行 = 講師)**、ヘッダは
    「曜日 / 学年 / コース / クラス / 教室」の 5 段
  - **隔週コマは対角罫で仕切った 2×2**。左上が A 週 (講師欄の主担当)、
    右下が B 週 (note「隔週(◯◯)」のパートナー) — CLAUDE.md の
    「隔週コマの担当週」と同じ向き
  - **合同コマは構成クラスの上に結合**して被せる (配置は画面と共有の
    `mergedColumns.computeRowCells`)。確認テストのように担任つきで見せる
    コマは**時限まるごと 1 セル**にし、並列の監督は担任行を並べる。
    学年をまたぐ枠 (附中の確認テスト) は、中身がまったく同じブロックが
    隣り合っていれば 1 枠に結合する
  - **表の束ね方は「時間軸を共有する曜日をひとつの表に」**。附中 (16:25〜)
    や土曜の内申対策 (10:00〜) は本科 (18:00〜) と時刻が交わらないので
    自然に別の表になり、元 xls と同じ切れ方 (月火水 / 附中・木金 / 土) で
    左上から段組みに詰まる
  - クラス名がどこにも無い表 (附中のように教室だけで見せる) は
    **「クラス」段と「教室」段を 1 つに畳む**
- **「🎒📂」**で同じレイアウトの Excel を**新しいプロジェクトとして取込**
  できる。セル結合そのものを構造として読むので、列幅・時限の高さ・
  クラス段の有無を決め打ちせず、例年の手組みの紙面も読める。
  - 学年 × コースの組が 1 タブになる (同じ組が複数の曜日にあれば 1 つに
    まとまる)。合同コマは「S〜C」の範囲ラベル列に戻し、並列の担任は
    その列を人数ぶん作って振り分ける
  - **exceljs は .xlsx しか読めない**ため、.xls を選ぶと「Excel で .xlsx に
    保存し直してください」と案内する
- 学年タブに**「コース」欄**を追加した (⚙ の学年設定)。掲示紙面のコース行に
  出る。空欄ならタブ名から学年の接頭辞を落としたものを使うので、既存の
  プロジェクトはそのままでも出力できる。
