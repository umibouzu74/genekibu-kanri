...
- 合同 (複数クラス 1 コマ) は科目別シートと同じ「代表クラスに 1 回」の
  ...
  (P1) と同じ。
- **各講師の個人シートにも自分の分を掲載** (S3)。タイトル直下に
  「クラス別回数: ３S 5(テ2)、３A 3 ／ 計 8(テ2)」の 1 行まとめが入り、
  印刷して個人に渡った紙 1 枚で座席表・確認テストの必要枚数が分かる。
  タブが複数あるときはクラス名にタブ名を前置。まとめ行は Print_Titles に
  含まれるので 2 ページ目以降にも載る。
- 集計は純粋関数 `computeTeacherClassCounts` (utils/excelExport) に
  切り出し。テスト +15 (xlsx round-trip 検証含む)。
