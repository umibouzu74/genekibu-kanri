...
### Fixed (コース別 終講日: 授業の無い曜日がラベルに出る問題)
`deriveCohortsFromSlots` (`src/utils/cohorts.js`) のラベルが `dayPairLabel`
(火→火木 / 水→水金) をそのまま使っていたため、片曜日しか授業が無い学年で
存在しない曜日が表示されていた。例: 中1 は火・金のみだが「中1 火木」「中1 水金」、
附中は水のみだが「附中 水金」と表示。

- ラベルと `days` を「実際に授業がある曜日」だけから生成するよう修正
  (中1 火 / 中1 金 / 附中 水)。両曜日揃う場合は従来どおり「中3 火木」。
- グルーピング用のコホート ID (`slotCohortId` = ペア基準) は不変のため、
  保存済み終講日 (`displayCutoff.cohorts`) の照合・既存データへの影響は無し。
