...
- **◫ 曜日を並べる**: 選んだ複数の曜日 (例: 火と木) の表を横に並べて
  ...
  (RegularGrid の stackSections — 左右の曜日で同じ部が横に並ぶ。通常の
  曜日ビューは従来どおり 2 カラム流し込み・タブ定義順)。曜日カラムは
  折り返さない等幅 (flex-nowrap + flex-1、最小幅 320px) で固定 — セルを
  編集モードにしてプルダウン分だけ表が広がっても、カラム幅は変わらず
  セクション内の横スクロールで吸収されるので、編集のたびに右の曜日が
  下へ落ちて縦一列に崩れない。クラス列の最小幅の下限も通常ビューより
