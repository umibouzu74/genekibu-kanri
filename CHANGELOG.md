...
モーダルの枠 (タブ切替・フォーカストラップ・キーボードナビ) と 5 つのタブ
本体 (`regular-builder/config/{Periods,Subjects,Teachers,Rooms,Limits}Tab.jsx`)
に分けた。共通の小物 (chip の削除ボタンの class・講師 1 人の更新ラッパ・
配列の入れ替え) は `config/shared.js`。

- 分割前にスモークテスト (`ProjectConfigModal.test.jsx`) を足し、5 タブの
  描画と主な操作 (時限追加・科目追加・講師の追加/よみ/担当科目/本体からの
  取込・教室の取込・NG と上限の設定) が同じ保存結果になることを固定した
- 入力欄の打ちかけ (追加前の科目名や NG フォームの選択) はタブの中の state
  になったので、タブを切り替えると消える。保存済みの設定は変わらない

### Changed (休講の判定と分類を 1 か所に寄せた。挙動は同じ)

「この休講がこのコマに効くか」と「その日は全日休講か / 部単位か / 学年・
教科限定か」を、日別ダッシュボード・タイムテーブル・月次カレンダー・欠勤
組み換え・バイトの出勤日・隔週の週送りがそれぞれ書いていた。
`utils/scheduleHelpers` に集約した:

- `holidayAppliesTo(h, grade, subj)` — 部 → 学年 → 教科キーワードの順の
  適用判定 (ダッシュボードの `isHolidayForSlot` と `isSlotCancelledByHoliday`
  が同じものを別々に持っていた)
- `classifyDayHolidays(hols)` — `fullOff` / `offDepts` / `granularHols` /
  `hasPartial` (3 画面で同じ分岐を書いていた)
- `describeHolidayTargets` / `formatHolidayRange` — 「中3・高松西」「中学部 /
  中3 / 高松西」の文言 (4 画面で書き起こしていた)
- 複合学年 (中1-3 のプレップ) に「中3 だけの休講」を当てるかは決めておらず、
  従来どおり完全一致のまま。判断が要る点としてコードにも注記した

### Changed (開始日 > 終了日 の入力を防ぐ)

- **時間割管理の時間割フォームと複製ダイアログ**で、終了日が開始日より前
  なら「終了日は開始日以降にしてください」を出して保存・複製を止める
  (テスト期間・特別イベントには前からあった検証を揃えた)。逆転した期間の
  時間割は日付ベースの画面でどの日にも出ないので、保存前に気付ける
- 期間を持つ日付入力 (時間割・複製・テスト期間・特別イベント・表示期間設定の
  学年グループと一括設定) に `min` / `max` を付け、ブラウザの日付ピッカーで
  範囲外の日を選べないようにした
