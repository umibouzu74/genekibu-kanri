...
### Fixed (コース別 終講日: 授業の無い曜日が出る / コース単位でまとめる)
...
- **コース単位の導出** (`deriveCohortsFromSlots`, `partitionDaysIntoCourses`):
  中学は学年ごとに「実際に授業がある曜日」でコースを束ねる。火木・水金の
  両ペアが揃う学年 (中3) だけ 2 コースに分割し、それ以外は平日を 1 コースに
  まとめる (中1=火金 / 中2=月木 / 附中=水)。土曜は常に別コース。
- **ID に実曜日を埋め込む**: `M|<学年>|<曜日列>` (例 `M|中1|火金`)。
