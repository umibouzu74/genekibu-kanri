...
## 期切替 (前期/後期) の運用 (2026-08-01 確定)

- 期の切替は**新システムを作らず既存の時間割 (Timetable) 機構で行う**:
  時間割管理で現行を「2026 前期」に改名して終了日を設定 → 複製で
  「2026 後期」を作成して開始日を設定 → ヘッダのプルダウンで切替
  (日付ビューは期間で自動切替)。講習時間割作成 (builder) を通常時間割の
  作成に流用する案は却下 (日付ベースモデルで教室・隔週・並列コマを
  表現できず、単一プロジェクト前提のため)。
- 時刻だけ変わる期切替は時間割管理の**「時刻一括変換」**
  (`utils/timeBulkEdit` + `TimeBulkEditPanel`) を使う。
